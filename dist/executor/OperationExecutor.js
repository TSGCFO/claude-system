...
export class OperationExecutor {
    ...
    async executeFileOperation(operation) {
        const { action, path, content } = operation.params;
     ...
     */
    async executeWebNavigation(operation) {
        const { action, url, selector, coordinates } = operation.params;
     ...
     */
    async executeAppControl(operation) {
        const { action, appName } = operation.params;
     ...
     */
    async executeSystemSettings(operation) {
        const { action, setting } = operation.params;
    ...
    async executeCommand(operation) {
        const { command, cwd, env } = operation.params;
    ...
    async validateFileOperation(operation) {
        const { action, path } = operation.params;
        if (!path) {
            return {
                valid: false,
                errors: [{
     ...
     */
    validateWebNavigation(operation) {
        const { action, url } = operation.params;
     ...
     */
    validateSystemSettings(operation) {
        const { action, setting } = operation.params;
        if (!setting) {
            return {
                valid: false,
