...
dotenv.config();
const { CLAUDE_SYSTEM_USERNAME, CLAUDE_SYSTEM_PASSWORD, JWT_SECRET = 'default-secret-change-this' } = process.env;
...
export class SecurityManager {
    ...
    async authenticate(credentials) {
        ...
        // Create JWT token
        const token = jwt.sign({ userId, sessionId }, JWT_SECRET, { expiresIn: '24h' });
        const session = {
            id: sessionId,
            userId,
            createdAt: new Date(),
     ...
     */
    async authorizeOperation(operation, user) {
     ...
     */
    async validateResourceAccess(userId, resourceType, resourceId, action) {
     ...
     */
    startSessionCleanup(intervalMs = 60 * 60 * 1000) {
        setInterval(() => {
