import { NLIRequest, NLIResponse, Operation, OperationType } from '../types/core.js';
...
export class NaturalLanguageInterface {
   ...
   */
  async processInput(request: NLIRequest): Promise<NLIResponse> {
    try {
   ...
   */
  private findCommandMatches(text: string): Array<{ pattern: CommandPattern; matches: RegExpMatchArray }> {
    const matches: Array<{ pattern: CommandPattern; matches: RegExpMatchArray }> = [];
   ...
   */
  private calculateConfidence(matches: RegExpMatchArray): number {
