...
export class QualityController {
  ...
  private readonly qualityLogsDir: string;
  ...
  public async validateResponse(response: any): Promise<ValidationResult> {
    try {
      const results = await Promise.all(
        this.validationRules.map(rule => rule.validate(response))
      );
  ...
  private async calculateAccuracy(response: any): Promise<number> {
  ...
  private async calculateCompleteness(response: any): Promise<number> {
  ...
  private async calculateConsistency(response: any): Promise<number> {
    ...
    try {
      ...
      for (const file of recentLogs) {
        const content = await fs.readFile(
          path.join(this.qualityLogsDir, file),
          'utf-8'
        );
